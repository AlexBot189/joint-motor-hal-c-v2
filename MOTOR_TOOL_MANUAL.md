...
# 1. 启动 daemon
sudo ./motor_tool daemon can0 &

# 2. 等待电机上线后启动（auto_enable 或手动）
#    auto_enable: 电机1发 bootup → daemon 自动 startup → 打印 OPERATION_ENABLED
#    手动触发:
...
# 4. 校准（单电机写法）
./motor_tool calib start 1          # 默认10s超时
./motor_tool calib start 1 0 20000  # 自定义20s超时（id_l=0跳过）

# 5. 控制
./motor_tool torque 1 500           # 500mA 电流
./motor_tool speed 1 30             # 30 RPM
./motor_tool abs 1 45               # 绝对位置 45°

# 6. 持续监控
./motor_tool watch 200              # 200ms 周期，Ctrl+C 退出

# 7. 传感器透传
./motor_tool sensor config 1 250    # 250Hz 透传
./motor_tool sensor watch 1         # 看板模式
...
# 广播启动: ./motor_tool startup 0   (尝试启动 1~4)

# 3. 校准（双电机）
./motor_tool calib start 1 2        # 左右电机一起校准
./motor_tool calib start 1 2 20000  # 自定义超时

# 4. 左右分别控制
./motor_tool torque 1 500           # 右腿 500mA
./motor_tool torque 2 300           # 左腿 300mA
./motor_tool speed 1 50             # 右腿 50RPM
./motor_tool abs 2 -30              # 左腿 -30°

# 5. PDO 多轴广播
./motor_tool multi cur 1:500 2:300  # 广播电流
./motor_tool multi vel 1:50 2:-30   # 广播速度
...
./motor_tool stop
拔掉任意电机的灵活切换
V2 patch 打上后，daemon 照常注册电机 1 和 2（不管接没接）。拔掉的电机不发 bootup → 不触发 auto_enable → 不对它发 SDO → 不影响在线的电机。
场景: 只接电机2（电机1拔了）
  sudo ./motor_tool daemon can0 &
  ./motor_tool startup 2
  ./motor_tool calib start 2
  ./motor_tool torque 2 500
  ./motor_tool watch 200
场景: 先接电机1，测完拔掉换电机2
  motor_tool stop                 # 停daemon
  # ... 拔电机1，接电机2 ...
  sudo ip link set can0 down      # 重置CAN(清error计数器)
  sudo ip link set can0 up
  sudo ./motor_tool daemon can0 & # 重开daemon
  ./motor_tool startup 2
