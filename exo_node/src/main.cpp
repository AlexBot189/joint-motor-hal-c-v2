...
int main(int argc, char** argv)
{
    ...
    while (g_running) {
        ...
        if (g_rt_worker && shm) {
            ...
            /* FAULT 状态自动恢复: RT 线程检测到算法重连时清 severity → 主线程切 READY */
            if (g_exo_state == STATE_FAULT && shm->motor_severity == MOTOR_OK) {
                ECO_INFO_NEW("[main] FAULT → READY (algo reconnected, severity=OK)");
