    ...
    while (running) {
        ...
        if (fb->timestamp_us > 0) {
            ...
            if (state == STATE_RUNNING && !initialized) {
                ...
                cmd_set(shm, 1, 2, STARK_CMD_ENABLE,   0, 0, 0, 0);
                seq++;
                ...
                cmd_set(shm, 1, 2, STARK_CMD_SET_MODE, ALGO_MODE_CURRENT, 0, 0, 0);
                seq++;
                __atomic_store_n(&shm->mailbox.seq_begin, seq, __ATOMIC_RELEASE);
                shm->mailbox.seq_end = seq;
                usleep(5000);
            ...
            if (t_sec < 3.0f) {
                /* ── 力矩控制 ── */
                if (online & 0x01) {
                    cmd_set(shm, 0, 1, STARK_CMD_TORQUE, 200, 0, 0, fb->timestamp_us);
                }
                if (online & 0x02) {
                    cmd_set(shm, 1, 2, STARK_CMD_TORQUE, 200, 0, 0, fb->timestamp_us);
                }

                /* 也可以走多轴广播 (取消注释体验一帧64B发出双电机):
                if (online & 0x01) cmd_set(shm, 0, 1, STARK_CMD_MULTI, 200, 0, 0, fb->timestamp_us);
                if (online & 0x02) cmd_set(shm, 1, 2, STARK_CMD_MULTI, 200, 0, 0, fb->timestamp_us);
                */
                ...
                if (!pos_mode_set) {
                    ...
                    cmd_set(shm, 1, 2, STARK_CMD_SET_MODE, ALGO_MODE_CSP, 0, 0, 0);
                    seq++;
                    __atomic_store_n(&shm->mailbox.seq_begin, seq, __ATOMIC_RELEASE);
                    shm->mailbox.seq_end = seq;
                    usleep(5000);
                ...
                if (!mit_mode_set) {
                    ...
                    cmd_set(shm, 1, 2, STARK_CMD_SET_MODE, ALGO_MODE_MIT, 0, 0, 0);
                    seq++;
                    __atomic_store_n(&shm->mailbox.seq_begin, seq, __ATOMIC_RELEASE);
                    shm->mailbox.seq_end = seq;
                    usleep(5000);
                ...
                if (online & 0x02) {
                    ...
                    shm->mailbox.cmd[1].cmd          = STARK_CMD_MIT;
...
next_cycle:
    ...
    cmd_set(shm, 1, 2, STARK_CMD_ESTOP, 0, 0, 0, 0);
    seq++;
    __atomic_store_n(&shm->mailbox.seq_begin, seq, __ATOMIC_RELEASE);
    shm->mailbox.seq_end = seq;
    usleep(20000);
