 ...
 *   2. 算法发 STARK_CMD_ENABLE ,  pdo_byte0 bit7=1 ,  电机开始响应PDO
 *   3. 算法发 STARK_CMD_SET_MODE ,  切换控制模式
 *   4. 控制循环发 STARK_CMD_TORQUE/POS/SPEED/MIT ,  只传target
 *   5. 急停: STARK_CMD_ESTOP
 *   6. 恢复: STARK_CMD_RECOVER
 *   7. 清错: 先读 fb->motor[i].error_code ,  判断 ,  STARK_CMD_CLEAR_FAULT
 *
 * Byte0 字段说明:
 *   bit7: PDO使能 (STARK_CMD_ENABLE/DISABLE)
 *   bit6: 母线电压 (预留, 当前电机不实现机械抱闸)
 *   bit5: 清错脉冲 (STARK_CMD_CLEAR_FAULT, 上层先读 error_code 再决定)
 *   bit4:1: 控制模式 (STARK_CMD_SET_MODE)
 *
...
int main()
{
    ...
    int fd = shm_open(STARK_SHM_NAME, O_RDWR, 0666);
    if (fd < 0) {
        perror("[algo_sim] shm_open: start stark_periph_manager_node first");
        return 1;
    }

    stark_shm_t* shm = (stark_shm_t*)mmap(NULL, STARK_SHM_SIZE,
    ...
    while (running) {
        ...
        if (fb->timestamp_us > 0) {
            ...
            if (state == STATE_RUNNING && !initialized) {
                ...
                cmd_set(shm, 1, 2, seq, STARK_CMD_ENABLE,   0, 0, 0, 0);
                seq++;
shm->mailbox.seq_write = seq;
                usleep(5000);

                cmd_set(shm, 0, 1, seq, STARK_CMD_SET_MODE, ALGO_MODE_CURRENT, 0, 0, 0);
                cmd_set(shm, 1, 2, seq, STARK_CMD_SET_MODE, ALGO_MODE_CURRENT, 0, 0, 0);
                seq++;
shm->mailbox.seq_write = seq;
                usleep(5000);
                initialized = true;
            ...
            if (t_sec < 3.0f) {
                /* ── 力矩控制 ── */
                if (online & 0x01) {
                    cmd_set(shm, 0, 1, seq, STARK_CMD_TORQUE, 200, 0, 0, fb->timestamp_us);
                }
                if (online & 0x02) {
                    cmd_set(shm, 1, 2, seq, STARK_CMD_TORQUE, 200, 0, 0, fb->timestamp_us);
                }

                /* 也可以走多轴广播 (取消注释体验一帧64B发出双电机):
                if (online & 0x01) cmd_set(shm, 0, 1, seq, STARK_CMD_MULTI, 200, 0, 0, fb->timestamp_us);
                if (online & 0x02) cmd_set(shm, 1, 2, seq, STARK_CMD_MULTI, 200, 0, 0, fb->timestamp_us);
                */
                ...
                if (!pos_mode_set) {
                    ...
                    cmd_set(shm, 1, 2, seq, STARK_CMD_SET_MODE, ALGO_MODE_CSP, 0, 0, 0);
                    seq++;
shm->mailbox.seq_write = seq;
                    usleep(5000);
                    pos_mode_set = true;
                ...
                if (online & 0x02) {
                    cmd_set(shm, 1, 2, seq, STARK_CMD_POS,
                            (int32_t)(-angle * 100.0f), 0, 0, fb->timestamp_us);
                }
                ...
                if (!mit_mode_set) {
                    ...
                    cmd_set(shm, 1, 2, seq, STARK_CMD_SET_MODE, ALGO_MODE_MIT, 0, 0, 0);
                    seq++;
shm->mailbox.seq_write = seq;
                    usleep(5000);
                    mit_mode_set = true;
                ...
                if (online & 0x01) {
                    ...
                    shm->mailbox.frames[seq % STARK_MBOX_DEPTH].cmd[0].mit_pos      = 32768;   /* 中间 */
                    shm->mailbox.frames[seq % STARK_MBOX_DEPTH].cmd[0].mit_vel      = 0;
                    shm->mailbox.frames[seq % STARK_MBOX_DEPTH].cmd[0].mit_kp       = 50;      /* 低刚度 */
                    shm->mailbox.frames[seq % STARK_MBOX_DEPTH].cmd[0].mit_kd       = 5;       /* 低阻尼 */
                ...
                if (online & 0x02) {
                    shm->mailbox.frames[seq % STARK_MBOX_DEPTH].cmd[1].motor_id     = 2;
                    shm->mailbox.frames[seq % STARK_MBOX_DEPTH].cmd[1].cmd          = STARK_CMD_MIT;
                    shm->mailbox.frames[seq % STARK_MBOX_DEPTH].cmd[1].mit_pos      = 32768;
                    shm->mailbox.frames[seq % STARK_MBOX_DEPTH].cmd[1].mit_vel      = 0;
                    shm->mailbox.frames[seq % STARK_MBOX_DEPTH].cmd[1].mit_kp       = 50;
                    shm->mailbox.frames[seq % STARK_MBOX_DEPTH].cmd[1].mit_kd       = 5;
                    shm->mailbox.frames[seq % STARK_MBOX_DEPTH].cmd[1].mit_torque   = 0;
                    shm->mailbox.frames[seq % STARK_MBOX_DEPTH].cmd[1].timestamp_us = fb->timestamp_us;
                }
                    ...
                    for (int i = 0; i < STARK_MAX_MOTORS; i++) {
                        ...
                        if (err & ALGO_ERR_OVER_TEMP) {
                            ...
                            if (shm->motor_online & (1 << i)) {
                                cmd_set(shm, i, (uint8_t)(i + 1), seq,
                                        STARK_CMD_CLEAR_FAULT, 0, 0, 0, fb->timestamp_us);
                            }
                        } else if (err != 0) {
...
next_cycle:
    ...
    cmd_set(shm, 1, 2, seq, STARK_CMD_ESTOP, 0, 0, 0, 0);
    seq++;
shm->mailbox.seq_write = seq;
    usleep(20000);
