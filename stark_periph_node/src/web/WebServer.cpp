 ...
 * ## 启用 crow 完整实现的步骤
 *
 *   1. 将 crow_all.h 放入 stark_periph_node/3rd/crow/ 或系统 include 路径
...
WebServer::WebServer(stark_shm_t* shm, uint16_t port)
    : m_shm(shm)
    , m_port(port)
    , m_running(false)
    , m_frame_count(0)
...
void WebServer::PullLoop()
{
    ...
    ([]{
        return R"(
<!DOCTYPE html>
<html><head><meta charset="utf-8"><title>Stark Debug</title></head>
<body>
<h1>Stark Node WebSocket Debug</h1>
<pre id="log"></pre>
<script>
