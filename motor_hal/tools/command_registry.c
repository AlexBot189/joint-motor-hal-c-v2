...
const command_entry_t g_cmd_table[] = {
    ...
    /* SDO 控制 — 自动使能 + 切模式 + 写目标 */
    { CMD_SDO,      "sdo",      "sdo cur <N> <mA>  |  sdo cur <N1> <N2> <mA>", "SDO电流控制(自动使能)", -1, -1 },

    /* SDO 配置 */
    { CMD_SETZERO,  "setzero",  "setzero <id>",                  "零位标定",                     2, 2 },
    { CMD_LIMIT_POS,"limit_pos","limit_pos <id> <deg*100>",     "正限位",                       3, 3 },
    { CMD_LIMIT_NEG,"limit_neg","limit_neg <id> <deg*100>",     "负限位",                       3, 3 },
    ...
    { CMD_PDO,   "pdo",   "pdo cur <N> <mA>  |  pdo cur <N1> <N2> <mA>", "PDO电流控制(自动使能+多轴广播)", -1, -1 },
...
int cmd_dispatch(motor_hal_t *hal, int argc, char **argv)
{
    ...
    switch (cmd->id) {
        ...
        case CMD_SDO:      return cmd_do_sdo(hal, cmd->id, argc, argv);
