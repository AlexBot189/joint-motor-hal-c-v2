...
## [SDO-001] 同一电机并发 SDO 读写会误杀响应帧 (2026-06-11)

**状态**: 已知, 未修复

**影响范围**: `src/sdo_client.c` — `_sdo_wait_response` / `sdo_push_response`

**问题描述**:

两个线程同时对同一电机(ID)发起 SDO 读不同对象时, 响应帧会被误杀。
SDO 同一电机的收/发共用 COB-ID (0x580+ID), 队列用 COB-ID + index 匹配。
当前 `_sdo_wait_response` 扫描到同 COB-ID 但不同 index 的帧时直接丢弃,
导致另一线程的响应丢失。
...
## CAN 总线丢帧与缓冲区问题排查 (2026-06-11)

### 可能原因

| 原因 | 触发条件 | 现象 |
|------|---------|------|
| 总线仲裁冲突 | 高优先级帧持续占总线 | SDO 超时, PDO 帧丢失 |
| CAN 控制器 FIFO 满 | 接收帧速率超过消费速度 | `overrun` 计数增加 |
| SocketCAN 内核缓冲满 | `txqueuelen` 太小 | `write: No buffer space available` |
| CANFD BRS 位同步失败 | 线缆过长 / 终端电阻不当 | 偶发 CRC 错误帧 |
...
关注: `errors`(总线错误)、`dropped`(内核丢帧)、`overrun`(FIFO 溢出)

**2. candump 过滤**

```
candump can0,0x601:0xFFF,0x581:0xFFF    # 只看 SDO
candump can0 -ta -x                       # 带时间戳
```
...
| 帧类型 | 每帧时间 | 频率 | 负载 |
|--------|:---:|:---:|:---:|
| 心跳(1Hz) | ~130μs | 1Hz | ~0.01% |
| 反馈(100Hz) | ~130μs | 100Hz | ~1.3% |
| PDO(1KHz) | ~130μs | 1KHz | ~13% |
| 多轴广播 64B | ~200μs | 1KHz | ~20% |

### 当前场景结论

- SDO 有 3 次重试 + 超时, 不担心丢帧
- PDO 天生容忍丢帧 (下一帧覆盖上一帧)
- SocketCAN 吞吐上限 >> 当前负载
- 断线检测靠心跳已实现 (SafetyCheck)
- 不需要额外做丢帧防护
